...
def build_pdf_bytes(df_active: pd.DataFrame, df_history: pd.DataFrame, df_new_active: pd.DataFrame) -> bytes:
    ...
    def parse_note_groups(raw: str):
        """แปลง note string เป็น dict {date: [line, ...]} เรียงวันใหม่ก่อน"""
        ...
        for line in raw.splitlines():
            ...
            if not line:
                continue
            # รูปแบบ [DD/MM/YYYY] ข้อความ
            import re
            m = re.match(r'^\[(\d{2}/\d{2}/\d{4})\]\s*(.*)', line)
            if m:
                d, text = m.group(1), m.group(2).strip()
                groups.setdefault(d, []).append(text)
            else:
                no_date.append(line)
        if no_date:
            groups.setdefault("—", []).extend(no_date)
        return groups
...
"""
                ...
                def format_note_for_display(raw: str) -> str:
                    """จัดกลุ่ม note ตามวันที่ แต่ละ entry คั่นด้วยบรรทัดว่าง 2 บรรทัด"""
                    import re
                    from collections import OrderedDict
                ...
                def prep_display(df: pd.DataFrame) -> pd.DataFrame:
                    """คืน df ที่ format คอลัมน์ note แล้ว"""
                    d = df.copy()
                    if NOTE_COL in d.columns:
                        d[NOTE_COL] = d[NOTE_COL].apply(
                            lambda v: format_note_for_display(str(v))
                        )
                    return d
                ...
                _col_cfg = {
                    NOTE_COL: st.column_config.TextColumn(
                        NOTE_COL, width="large"
                    ),
                    "หัวข้อปัญหา": st.column_config.TextColumn("หัวข้อปัญหา", width="medium"),
                }
                ...
                if not df_followup.empty:
                    st.dataframe(prep_display(df_followup), use_container_width=True,
                ...
                if not df_new_active.empty:
                    ...
                    st.dataframe(prep_display(df_new_active), use_container_width=True,
                                 column_config=_col_cfg, hide_index=True)

                st.subheader(f"📜 งานที่สิ้นสุดแล้ว (History) — {len(df_history)} รายการ")
                if not df_history.empty:
                    st.dataframe(prep_display(df_history), use_container_width=True,
                                 column_config=_col_cfg, hide_index=True)
