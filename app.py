...
try:
    ...
    email_recipients = st.secrets.get("EMAIL_RECIPIENTS", "atichat@tspmetal.com")
except Exception:
...
def build_pdf_bytes(df_active: pd.DataFrame, df_history: pd.DataFrame, updates: dict) -> bytes:
    ...
    def make_table_html(df, title):
        if df.empty:
            return f"<h3>{title}</h3><p class='no-data'>ไม่มีรายการในขณะนี้</p>"
        cols = ['Issue ID', 'หัวข้อปัญหา', 'ผู้รับผิดชอบ', 'สถานะ', 'กำหนดเสร็จ', 'วันที่แจ้ง', 'งานที่ทำไปแล้ว / อุปสรรค / Resource ที่ต้องการเพิ่ม']
        existing = [c for c in cols if c in df.columns]
        headers = "".join(f"<th>{c}</th>" for c in existing)
...
<style>
  ...
  * {{ box-sizing: border-box; margin:0; padding:0; }}
  ...
  .header h1 {{ color:#1a5c3a; font-size:20px; font-weight:700; margin-bottom:4px; }}
  .header h2 {{ font-size:14px; color:#2d7a56; font-weight:500; margin-bottom:4px; }}
  ...
  /* 7 คอลัมน์ แนวนอน */
  th:nth-child(1), td:nth-child(1) {{ width: 6%; }}   /* Issue ID */
  th:nth-child(2), td:nth-child(2) {{ width: 18%; }}  /* หัวข้อ */
  th:nth-child(3), td:nth-child(3) {{ width: 10%; }}  /* ผู้รับผิดชอบ */
  th:nth-child(4), td:nth-child(4) {{ width: 8%; }}   /* สถานะ */
  th:nth-child(5), td:nth-child(5) {{ width: 9%; }}   /* กำหนดเสร็จ */
  th:nth-child(6), td:nth-child(6) {{ width: 9%; }}   /* วันที่แจ้ง */
  th:nth-child(7), td:nth-child(7) {{ width: 40%; }}  /* งานที่ทำไปแล้ว */
...
<div class='summary'>
  📌 งานคงค้าง: <strong>{len(df_active)} รายการ</strong> &nbsp;&nbsp; 
  ✅ งานปิดแล้ว: <strong>{len(df_history)} รายการ</strong>
</div>
...
"""
                ...
                if not df_active.empty:
                    for _, row in df_active.iterrows():
                        ...
                        line_msg += f"• {issue_id}: {row['หัวข้อปัญหา']} ({row['ผู้รับผิดชอบ']})\n"
                        # แสดงความคืบหน้าที่เพิ่งอัปเดต
                        if issue_id in updates and updates[issue_id]["note"].strip():
                            line_msg += f"  └ 📝 {updates[issue_id]['note'].strip()}\n"
                        # แสดงสถานะ
                        line_msg += f"  └ 📌 สถานะ: {row['สถานะ']} | 🎯 กำหนดเสร็จ: {row.get('กำหนดเสร็จ', '-')}\n"

                line_msg += f"\n✅ งานปิดแล้ว: {len(df_history)} รายการ"
                if not df_history.empty:
                    for _, row in df_history.iterrows():
                        line_msg += f"\n  ✓ {row['Issue ID']}: {row['หัวข้อปัญหา']}"
